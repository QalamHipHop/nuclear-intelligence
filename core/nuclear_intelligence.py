...
Free provider chain: DeepSeek → Groq → Gemini → Together → OpenRouter → HuggingFace
═══════════════════════════════════════════════════════════════════
"""
...
class EvaluationScore:
    ...
    def overall_score(self) -> float:
        return (
            self.scientific_accuracy * 0.45 +
            self.novelty_score * 0.25 +
            self.usefulness_score * 0.20 +
...
Return JSON:
{
  ...
  "confidence_level": "high|medium|low"
    ...
    "synthetic_context": """You are a Nuclear Intelligence Context Synthesizer. Synthesize context from multiple sources.

Return JSON:
{
  "synthesized_context": "comprehensive synthesized context (300-1000 words)",
  "key_facts": ["fact1", "fact2", "fact3", "fact4", "fact5"],
  "confidence_level": "high|medium|low",
  "data_gaps": ["gap1", "gap2"]
}""",

    "critic": """You are a Nuclear Science Critic. Identify weaknesses and inconsistencies in research answers.
...
class NuclearIntelligenceCore:
    """Advanced AI Research Engine for Nuclear Intelligence"""
...
Provide a comprehensive, detailed scientific answer with equations, mechanisms, and real citations."""
        ...
        # Add citations from web results
...
Provide deep technical analysis.""",
        ...
        return {
            "error": "Developer analysis unavailable",
            "cross_domain": [],
            "research_gaps": [],
        }

    def critic_review(self, question: ResearchQuestion, answer: ResearchAnswer) -> Dict:
        """Critic review to identify weaknesses"""
        result = self.llm.structured_completion(
            prompt=f"Question: {question.question}\nAnswer: {answer.answer[:3000]}",
        ...
        if result and not result.get("parse_error"):
            return result.get("parsed", {})
        return {"weaknesses": [], "inconsistencies": [], "suggestions": []}
    ...
    def ask_question(self, question: str, developer_mode: bool = False, use_web_search: bool = True) -> Dict:
        ...
        if developer_mode:
            ...
            result["critic_review"] = self.critic_review(q, answer)
